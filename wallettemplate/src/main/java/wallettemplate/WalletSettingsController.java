...
public class WalletSettingsController {
    ...
    public void initialize(@Nullable KeyParameter aesKey) {
        DeterministicSeed seed = Main.neoscoin.wallet().getKeyChainSeed();
        if (aesKey == null) {
            if (seed.isEncrypted()) {
            ...
            this.aesKey = aesKey;
            seed = seed.decrypt(checkNotNull(Main.neoscoin.wallet().getKeyCrypter()), "", aesKey);
    ...
    public void restoreClicked(ActionEvent event) {
        ...
        if (Main.neoscoin.wallet().getBalance().value > 0) {
        ...
        informationalAlert("Wallet restore in progress",
                "Your wallet will now be resynced from the Neoscoin network. This can take a long time for old wallets.");
        overlayUI.done();
        ...
        // Shut down neoscoinj and restart it with the new seed.
        Main.neoscoin.addListener(new Service.Listener() {
            @Override
            public void terminated(Service.State from) {
                Main.instance.setupWalletKit(seed);
                Main.neoscoin.startAsync();
            }
        }, Platform::runLater);
        Main.neoscoin.stopAsync();
    }
    ...
    public void passwordButtonClicked(ActionEvent event) {
        if (aesKey == null) {
            ...
            Main.neoscoin.wallet().decrypt(aesKey);
