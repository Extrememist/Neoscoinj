...
/**
 * A custom control that implements a clickable, copyable Neoscoin address. Clicking it opens a local wallet app. The
 * address looks like a blue hyperlink. Next to it there are two icons, one that copies to the clipboard and another
 * that shows a QRcode.
 */
public class ClickableNeoscoinAddress extends AnchorPane {
    ...
    public ClickableNeoscoinAddress() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("neoscoin_address.fxml"));
            loader.setRoot(this);
    ...
    public String uri() {
        return NeoscoinURI.convertToNeoscoinURI(address.get(), null, Main.APP_NAME, null);
    }
    ...
    protected void showQRCode(MouseEvent event) {
        ...
        final Main.OverlayUI<ClickableNeoscoinAddress> overlay = Main.instance.overlayUI(pane, this);
