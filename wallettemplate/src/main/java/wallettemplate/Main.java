...
public class Main extends Application {
    ...
    public static WalletAppKit neoscoin;
    ...
    private void realStart(Stage mainWindow) throws IOException {
        ...
        // Tell neoscoinj to run event handlers on the JavaFX UI thread. This keeps things simple and means
        ...
        neoscoin.addListener(new Service.Listener() {
            @Override
    ...
    public void setupWalletKit(@Nullable DeterministicSeed seed) {
        ...
        neoscoin = new WalletAppKit(params, new File("."), APP_NAME + "-" + params.getPaymentProtocolId()) {
            @Override
            protected void onSetupCompleted() {
                // Don't make the user wait for confirmations for now, as the intention is they're sending it
                // their own money!
                neoscoin.wallet().allowSpendingUnconfirmedTransactions();
                Platform.runLater(controller::onNeoscoinSetup);
            }
        };
        // Now configure and start the appkit. This will take a second or two - we could show a temporary splash screen
        // or progress widget to keep the user engaged whilst we initialise, but we don't.
        if (params == RegTestParams.get()) {
            neoscoin.connectToLocalHost();   // You should run a regtest mode neoscoind locally.
        } else if (params == TestNet3Params.get()) {
            // As an example!
            neoscoin.useTor();
            // neoscoin.setDiscovery(new HttpDiscovery(params, URI.create("http://localhost:8080/peers"), ECKey.fromPublicOnly(BaseEncoding.base16().decode("02cba68cfd0679d10b186288b75a59f9132b1b3e222f6332717cb8c4eb2040f940".toUpperCase()))));
        }
        neoscoin.setDownloadListener(controller.progressBarUpdater())
               .setBlockingStartup(false)
               .setUserAgent(APP_NAME, "1.0");
        if (seed != null)
            neoscoin.restoreWalletFromSeed(seed);
    }
