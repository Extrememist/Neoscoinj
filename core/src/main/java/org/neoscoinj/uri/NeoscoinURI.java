...
import org.neoscoinj.core.AddressFormatException;
...
/**
 * <p>Provides a standard implementation of a Neoscoin URI with support for the following:</p>
 *
 * <ul>
 ...
 *
 * <ul>
 * <li>{@code neoscoin:<address>}</li>
 * <li>{@code neoscoin:<address>?<name1>=<value1>&<name2>=<value2>} with multiple
 ...
 * @see <a href="https://github.com/neoscoin/bips/blob/master/bip-0021.mediawiki">BIP 0021</a>
 */
public class NeoscoinURI {
    /**
     * Provides logging for this class
     */
    private static final Logger log = LoggerFactory.getLogger(NeoscoinURI.class);
    ...
    public static final String BITCOIN_SCHEME = "neoscoin";
    ...
    /**
     * Constructs a new NeoscoinURI from the given string. Can be for any network.
     *
     * @param uri The raw URI data to be parsed (see class comments for accepted formats)
     * @throws NeoscoinURIParseException if the URI is not syntactically or semantically valid.
     */
    public NeoscoinURI(String uri) throws NeoscoinURIParseException {
        this(null, uri);
    }

    /**
     * Constructs a new object by trying to parse the input as a valid Neoscoin URI.
     *
     ...
     *
     * @throws NeoscoinURIParseException If the input fails Neoscoin URI syntax and semantic checks.
     */
    public NeoscoinURI(@Nullable NetworkParameters params, String input) throws NeoscoinURIParseException {
        checkNotNull(input);
        ...
        try {
            ...
            throw new NeoscoinURIParseException("Bad URI syntax", e);
        }

        // URI is formed as  neoscoin:<address>?<query parameters>
        // blockchain.info generates URIs of non-BIP compliant form neoscoin://address?....
        // We support both until Ben fixes his code.
        
        // Remove the neoscoin scheme.
        // (Note: getSchemeSpecificPart() is not used as it unescapes the label and parse then fails.
        // For instance with : neoscoin:129mVqKUmJ9uwPxKJBnNdABbuaaNfho4Ha?amount=0.06&label=Tom%20%26%20Jerry
        // the & (%26) in Tom and Jerry gets interpreted as a separator and the label then gets parsed
        // as 'Tom ' instead of 'Tom & Jerry')
        String schemeSpecificPart;
        if (input.startsWith("neoscoin://")) {
            schemeSpecificPart = input.substring("neoscoin://".length());
        } else if (input.startsWith("neoscoin:")) {
            schemeSpecificPart = input.substring("neoscoin:".length());
        } else {
            throw new NeoscoinURIParseException("Unsupported URI scheme: " + uri.getScheme());
        }

        // Split off the address from the rest of the query parameters.
        String[] addressSplitTokens = schemeSpecificPart.split("\\?", 2);
        if (addressSplitTokens.length == 0)
            throw new NeoscoinURIParseException("No data found after the neoscoin: prefix");
        ...
        if (!addressToken.isEmpty()) {
            // Attempt to parse the addressToken as a Neoscoin address for this network
            try {
                Address address = new Address(params, addressToken);
                putWithValidation(FIELD_ADDRESS, address);
            } catch (final AddressFormatException e) {
                throw new NeoscoinURIParseException("Bad address", e);
            }
        }

        if (addressToken.isEmpty() && getPaymentRequestUrl() == null) {
            throw new NeoscoinURIParseException("No address and no r= parameter found");
        }
    }
     ...
     */
    private void parseParameters(@Nullable NetworkParameters params, String addressToken, String[] nameValuePairTokens) throws NeoscoinURIParseException {
        // Attempt to decode the rest of the tokens into a parameter map.
        for (String nameValuePairToken : nameValuePairTokens) {
            final int sepIndex = nameValuePairToken.indexOf('=');
            if (sepIndex == -1)
                throw new NeoscoinURIParseException("Malformed Neoscoin URI - no separator in '" +
                        nameValuePairToken + "'");
            if (sepIndex == 0)
                throw new NeoscoinURIParseException("Malformed Neoscoin URI - empty name '" +
                        nameValuePairToken + "'");
     ...
     */
    private void putWithValidation(String key, Object value) throws NeoscoinURIParseException {
        if (parameterMap.containsKey(key)) {
            throw new NeoscoinURIParseException(String.format("'%s' is duplicated, URI is invalid", key));
        } else {
    ...
    /**
     * The Neoscoin Address from the URI, if one was present. It's possible to have Neoscoin URI's with no address if a
     ...
     *         10,000,000 units is 1 NEOS. May be null if no amount is specified
     */
    ...
    public String toString() {
        StringBuilder builder = new StringBuilder("NeoscoinURI[");
        boolean first = true;
    ...
    public static String convertToNeoscoinURI(Address address, Coin amount, String label, String message) {
        return convertToNeoscoinURI(address.toString(), amount, label, message);
    }

    /**
     * Simple Neoscoin URI builder using known good fields.
     * 
     * @param address The Neoscoin address
     * @param amount The amount
     * @param label A label
     * @param message A message
     * @return A String containing the Neoscoin URI
     */
    public static String convertToNeoscoinURI(String address, @Nullable Coin amount, @Nullable String label,
