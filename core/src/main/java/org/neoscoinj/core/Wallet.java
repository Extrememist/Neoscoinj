...
import org.neoscoin.protocols.payments.Protos.PaymentDetails;
import org.neoscoinj.core.TransactionConfidence.ConfidenceType;
import org.neoscoinj.crypto.*;
import org.neoscoinj.params.UnitTestParams;
import org.neoscoinj.script.Script;
import org.neoscoinj.script.ScriptBuilder;
import org.neoscoinj.script.ScriptChunk;
import org.neoscoinj.signers.LocalTransactionSigner;
import org.neoscoinj.signers.MissingSigResolutionSigner;
import org.neoscoinj.signers.TransactionSigner;
import org.neoscoinj.store.FullPrunedBlockStore;
import org.neoscoinj.store.UnreadableWalletException;
import org.neoscoinj.store.WalletProtobufSerializer;
import org.neoscoinj.utils.BaseTaggableObject;
import org.neoscoinj.utils.ExchangeRate;
import org.neoscoinj.utils.ListenerRegistration;
import org.neoscoinj.utils.Threading;
import org.neoscoinj.wallet.*;
import org.neoscoinj.wallet.Protos.Wallet.EncryptionType;
import org.neoscoinj.wallet.WalletTransaction.Pool;
 ...
 * of the Neoscoin protocol.</p>
 *
 * <p>To learn more about this class, read <b><a href="https://neoscoinj.github.io/working-with-the-wallet">
 ...
 * other objects, see the <a href="https://neoscoinj.github.io/getting-started">Getting started</a> tutorial
 * on the website to learn more about how to set everything up.</p>
 *
 * <p>Wallets can be serialized using either Java serialization - this is not compatible across versions of neoscoinj,
 ...
 * {@link Wallet#autosaveToFile(java.io.File, long, java.util.concurrent.TimeUnit, org.neoscoinj.wallet.WalletFiles.Listener)}
...
public class Wallet extends BaseTaggableObject implements Serializable, BlockChainListener, PeerFilterProvider, KeyBag, TransactionBag {
     ...
     * {@link org.neoscoinj.wallet.KeyChain.KeyPurpose#RECEIVE_FUNDS}. The returned key is stable until
    ...
    /**
     * An alias for calling {@link #currentKey(org.neoscoinj.wallet.KeyChain.KeyPurpose)} with
     * {@link org.neoscoinj.wallet.KeyChain.KeyPurpose#RECEIVE_FUNDS} as the parameter.
     */
    ...
    /**
     * Returns address for a {@link #currentKey(org.neoscoinj.wallet.KeyChain.KeyPurpose)}
     */
    ...
    /**
     * An alias for calling {@link #currentAddress(org.neoscoinj.wallet.KeyChain.KeyPurpose)} with
     * {@link org.neoscoinj.wallet.KeyChain.KeyPurpose#RECEIVE_FUNDS} as the parameter.
     */
     ...
     * {@link org.neoscoinj.wallet.DeterministicKeyChain}. When the parameter is
     * {@link org.neoscoinj.wallet.KeyChain.KeyPurpose#RECEIVE_FUNDS} the returned key is suitable for being put
    ...
    /**
     * An alias for calling {@link #freshKey(org.neoscoinj.wallet.KeyChain.KeyPurpose)} with
     * {@link org.neoscoinj.wallet.KeyChain.KeyPurpose#RECEIVE_FUNDS} as the parameter.
     */
    ...
    /**
     * Returns address for a {@link #freshKey(org.neoscoinj.wallet.KeyChain.KeyPurpose)}
     */
    ...
    /**
     * An alias for calling {@link #freshAddress(org.neoscoinj.wallet.KeyChain.KeyPurpose)} with
     * {@link org.neoscoinj.wallet.KeyChain.KeyPurpose#RECEIVE_FUNDS} as the parameter.
     */
     ...
     * key, after loading a wallet produced by previous versions of neoscoinj. If the wallet is encrypted the key
     ...
     * If {@link Wallet#autosaveToFile(java.io.File, long, java.util.concurrent.TimeUnit, org.neoscoinj.wallet.WalletFiles.Listener)}
    ...
    /** See {@link org.neoscoinj.wallet.DeterministicKeyChain#setLookaheadSize(int)} for more info on this. */
    ...
    /** See {@link org.neoscoinj.wallet.DeterministicKeyChain#setLookaheadThreshold(int)} for more info on this. */
     ...
     * See {@link org.neoscoinj.wallet.DeterministicKeyChain#markKeyAsUsed(DeterministicKey)} for more info on this.
     */
     ...
     * @throws org.neoscoinj.core.ECKey.MissingPrivateKeyException if the seed is unavailable (watching wallet)
     */
    ...
    /**
     * Convenience wrapper around {@link Wallet#encrypt(org.neoscoinj.crypto.KeyCrypter,
     ...
     * {@link org.neoscoinj.crypto.KeyCrypterScrypt}.
     *
    ...
    private void processTxFromBestChain(Transaction tx, boolean forceAddToPool) throws VerificationException {
        ...
        if (isDeadCoinbase) {
            ...
            // we could. Neoscoin Core nodes on the network have deleted the dependent transactions from their mempools
        ...
        for (TransactionInput input : tx.getInputs()) {
            ...
            if (result == TransactionInput.ConnectionResult.ALREADY_SPENT) {
                if (fromChain) {
                    ...
                    log.warn("{}: {}", tx.getHash(), Utils.HEX.encode(tx.unsafeNeoscoinSerialize()));
                    Transaction other = input.getConnectedOutput().getSpentBy().getParentTransaction();
                    log.warn("{}: {}", other.getHash(), Utils.HEX.encode(tx.unsafeNeoscoinSerialize()));
                }
    ...
    public String toString(boolean includePrivateKeys, boolean includeTransactions, boolean includeExtensions,
        ...
        try {
            ...
            builder.append(String.format("Wallet containing %s NEOS (available: %s NEOS) in:%n",
     ...
     * of {@link org.neoscoinj.core.ECKey#getCreationTimeSeconds()}. This can return zero if at least one key does
     ...
     * you can use {@link org.neoscoinj.utils.Threading#waitForUserCode()} to block until the future had a
    ...
    public static class SendResult {
        /** The Neoscoin transaction message that moves the money. */
    ...
    public enum MissingSigsMode {
        ...
        USE_DUMMY_SIG,
        /**
         * If signature is missing, {@link org.neoscoinj.signers.TransactionSigner.MissingSignatureException}
    ...
    public static class SendRequest {
         ...
         * (its value is ignored and set to {@link org.neoscoinj.core.Wallet#getBalance()} - the fees required
         ...
         * is how mining is incentivized in later years of the Neoscoin system when inflation drops. It also provides
        ...
        /**
         * If not null, the {@link org.neoscoinj.wallet.CoinSelector} to use instead of the wallets default. Coin selectors are
     ...
     *
     * @param address The Neoscoin address to send the money to.
     ...
     * @throws ExceededMaxTransactionSize if the resultant transaction is too big for Neoscoin to process (try breaking up the amounts of value)
     */
    ...
    public void completeTx(SendRequest req) throws InsufficientMoneyException {
        ...
        try {
            ...
            // We need to know if we need to add an additional fee because one of our values are smaller than 0.01 NEOS
            ...
            // Check size.
            int size = req.tx.neoscoinSerialize().length;
    ...
    private boolean adjustOutputDownwardsForFee(Transaction tx, CoinSelection coinSelection, Coin baseFee, Coin feePerKb) {
        ...
        int size = tx.neoscoinSerialize().length;
                ...
                for (int i = 0; i < tx.getOutputs().size(); i++) {
                    ...
                    try {
                        if (isTxOutputBloomFilterable(out)) {
                            ...
                            filter.insert(outPoint.neoscoinSerialize());
                        }
                    } catch (ScriptException e) {
    ...
    public FeeCalculation calculateFee(SendRequest req, Coin value, List<TransactionInput> originalInputs,
        ...
        while (true) {
            ...
            // If change is < 0.01 NEOS, we will need to have at least minfee to be accepted by the network
            ...
            if (change.signum() > 0) {
                ...
                if (req.ensureMinRequiredFee && Transaction.MIN_NONDUST_OUTPUT.compareTo(change) >= 0) {
                    ...
                    size += changeOutput.neoscoinSerialize().length + VarInt.sizeOf(req.tx.getOutputs().size()) - VarInt.sizeOf(req.tx.getOutputs().size() - 1);
            ...
            size += req.tx.neoscoinSerialize().length;
     ...
     * sending transactions to the Neoscoin network by default. Some sendCoins methods let you specify a broadcaster
     ...
     */
    public void setTransactionBroadcaster(@Nullable org.neoscoinj.core.TransactionBroadcaster broadcaster) {
    ...
    private Transaction rekeyOneBatch(long timeSecs, @Nullable KeyParameter aesKey, List<Transaction> others, boolean sign) {
        ...
        try {
            ...
            checkState(rekeyTx.neoscoinSerialize().length < Transaction.MAX_STANDARD_TX_SIZE);
            return rekeyTx;
