 ...
 * the lightweight SPV mode, a {@link org.neoscoinj.store.SPVBlockStore} is the right choice.</p>
 *
 ...
 * {@link org.neoscoinj.store.H2FullPrunedBlockStore} implements full verification, which is equivalent to the
...
public abstract class AbstractBlockChain {
     ...
     *
     * Following this one down to the genesis block produces the story of the economy from the creation of Neoscoin
    ...
    private boolean add(Block block, boolean tryConnecting,
        ...
        try {
            ...
            // article here for more details: http://code.google.com/p/neoscoinj/wiki/SecurityModel
            try {
                block.verifyHeader();
        ...
        for (Transaction tx : transactions) {
            try {
                if (listener.isTransactionRelevant(tx)) {
                    ...
                    if (clone)
                        tx = new Transaction(tx.params, tx.neoscoinSerialize());
    ...
    private void checkTestnetDifficulty(StoredBlock storedPrev, Block prev, Block next) throws VerificationException, BlockStoreException {
        ...
        // There is an integer underflow bug in neoscoin-qt that means mindiff blocks are accepted when time
        // goes backwards.
