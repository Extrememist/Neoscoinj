...
import org.neoscoinj.wallet.DefaultRiskAnalysis;
import org.neoscoinj.wallet.KeyBag;
import org.neoscoinj.wallet.RedeemData;
...
public class TransactionInput extends ChildMessage implements Serializable {
    ...
    protected void neoscoinSerializeToStream(OutputStream stream) throws IOException {
        outpoint.neoscoinSerialize(stream);
     ...
     * "from address" is not well defined in Neoscoin and you should not assume that senders of a transaction can
     ...
     * in nodes memory pools if the existing version is time locked. See the Contracts page on the Neoscoin wiki for
     ...
     * @see TransactionOutPoint#getConnectedRedeemData(org.neoscoinj.wallet.KeyBag)
     */
    @Nullable
    ...
    public TransactionInput duplicateDetached() {
        return new TransactionInput(params, null, neoscoinSerialize(), 0);
    }

    /**
     * <p>Returns either RuleViolation.NONE if the input is standard, or which rule makes it non-standard if so.
     * The "IsStandard" rules control whether the default Neoscoin Core client blocks relay of a tx / refuses to mine it,
