...
import org.neoscoinj.net.AbstractTimeoutHandler;
import org.neoscoinj.net.MessageWriteTarget;
import org.neoscoinj.net.StreamParser;
...
public abstract class PeerSocketHandler extends AbstractTimeoutHandler implements StreamParser {
    ...
    private NeoscoinSerializer.NeoscoinPacketHeader header;
    ...
    public PeerSocketHandler(NetworkParameters params, InetSocketAddress remoteIp) {
        serializer = new NeoscoinSerializer(checkNotNull(params));
    ...
    public int receiveBytes(ByteBuffer buff) {
        checkArgument(buff.position() == 0 &&
                buff.capacity() >= NeoscoinSerializer.NeoscoinPacketHeader.HEADER_LENGTH + 4);
        try {
     ...
     * called automatically by {@link org.neoscoinj.net.NioClient} or
     * {@link org.neoscoinj.net.NioClientManager} once the socket finishes initialization.
     */
    @Override
