...
/**
 * <p>A Message is a data structure that can be serialized/deserialized using both the Neoscoin proprietary serialization
...
public abstract class Message implements Serializable {
     ...
     * @param protocolVersion Neoscoin protocol version.
    ...
    private void selfCheck(byte[] payload, int offset) {
        if (!(this instanceof VersionMessage)) {
            ...
            byte[] reserialized = neoscoinSerialize();
    ...
    // These methods handle the serialization/deserialization using the custom Neoscoin protocol.
    ...
    /**
     * Should only used by NeoscoinSerializer for cached checksum
     *
     * @return the checksum
     */
    byte[] getChecksum() {
    ...
    /**
     * Should only used by NeoscoinSerializer for caching checksum
     *
    ...
    /**
     * Returns a copy of the array returned by {@link Message#unsafeNeoscoinSerialize()}, which is safe to mutate.
    ...
    public byte[] neoscoinSerialize() {
        byte[] bytes = unsafeNeoscoinSerialize();
    ...
    /**
     * Serialize this message to a byte array that conforms to the neoscoin wire protocol.
     * <br/>
     ...
     */
    public byte[] unsafeNeoscoinSerialize() {
        ...
        try {
            neoscoinSerializeToStream(stream);
        } catch (IOException e) {
        ...
        if (parseRetain) {
            ...
            // which means another call to neoscoinSerialize is coming.  If we didn't recache then internal
    ...
    /**
     * Serialize this message to the provided OutputStream using the neoscoin wire format.
     *
     * @param stream
     * @throws IOException
     */
    final public void neoscoinSerialize(OutputStream stream) throws IOException {
    ...
    /**
     * Serializes this message to the provided stream. If you just want the raw bytes use neoscoinSerialize().
     */
    void neoscoinSerializeToStream(OutputStream stream) throws IOException {
        log.error("Error: {} class has not implemented neoscoinSerializeToStream method.  Generating message with no payload", getClass());
    }

    /**
     * This method is a NOP for all classes except Block and Transaction.  It is only declared in Message
     * so NeoscoinSerializer can avoid 2 instanceof checks + a casting.
     */
