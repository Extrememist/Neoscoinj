...
import org.neoscoinj.crypto.TransactionSignature;
...
import static org.neoscoinj.core.Utils.*;
import static com.google.common.base.Preconditions.checkState;

/**
 * <p>A transaction represents the movement of coins from some addresses to some other addresses. It can also represent
 * the minting of new coins. A Transaction object corresponds to the equivalent in the Neoscoin C++ implementation.</p>
 *
 * <p>Transactions are the fundamental atoms of Neoscoin and have many powerful features. Read
 * <a href="http://code.google.com/p/neoscoinj/wiki/WorkingWithTransactions">"Working with transactions"</a> in the
 * documentation to learn more about how to use this class.</p>
 *
 * <p>All Neoscoin transactions are at risk of being reversed, though the risk is much less than with traditional payment
...
public class Transaction extends ChildMessage implements Serializable {
    ...
    // These are serialized in both neoscoin and java serialization.
     ...
     * @param payload Neoscoin protocol formatted byte array containing message content.
    ...
    /**
     * Used by NeoscoinSerializer.  The serializer has to calculate a hash for checksumming so to
     ...
     * value is determined by a formula that all implementations of Neoscoin share. In 2011 the value of a coinbase
    ...
    public void clearInputs() {
        ...
        this.length = this.neoscoinSerialize().length;
    }
     ...
     * and requires external synchronization. Please refer to general documentation on Neoscoin scripting and contracts
    ...
    /**
     * Same as {@link #addSignedInput(TransactionOutPoint, org.neoscoinj.script.Script, ECKey, org.neoscoinj.core.Transaction.SigHash, boolean)}
     ...
     * a wrapper around calling {@link Transaction#hashForSignature(int, byte[], org.neoscoinj.core.Transaction.SigHash, boolean)}
    ...
    public synchronized Sha256Hash hashForSignature(int inputIndex, byte[] connectedScript, byte sigHashType) {
        ...
        //
        //   https://en.neoscoin.it/wiki/Contracts
        ...
        try {
            ...
            // is a legacy holdover from a previous, broken design of executing scripts that shipped in Neoscoin 0.1.
            ...
            if ((sigHashType & 0x1f) == (SigHash.NONE.ordinal() + 1)) {
                ...
                if (inputIndex >= this.outputs.size()) {
                    ...
                    // Neoscoin implementation. The reference client also contains a bug in handling this case:
    ...
    @Override
    protected void neoscoinSerializeToStream(OutputStream stream) throws IOException {
        uint32ToByteStreamLE(version, stream);
        stream.write(new VarInt(inputs.size()).encode());
        for (TransactionInput in : inputs)
            in.neoscoinSerialize(stream);
        stream.write(new VarInt(outputs.size()).encode());
        for (TransactionOutput out : outputs)
            out.neoscoinSerialize(stream);
     ...
     * since Neoscoin 0.8+ a transaction that did not end its lock period (non final) is considered to be non
     ...
     * This is useful in certain types of <a href="http://en.neoscoin.it/wiki/Contracts">contracts</a>, such as
