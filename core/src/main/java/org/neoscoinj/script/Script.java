 ...
 *
 * <p>Neoscoin transactions don't specify what they do directly. Instead <a href="https://en.neoscoin.it/wiki/Script">a
...
public class Script {
     ...
     * The concept of a "from address" isn't well defined in Neoscoin and you should not assume the sender of a
    ...
    private int findSigInRedeem(byte[] signatureBytes, Sha256Hash hash) {
        checkArgument(chunks.get(0).isOpCode()); // P2SH scriptSig
        int numKeys = Script.decodeFromOpN(chunks.get(chunks.size() - 2).opcode);
        TransactionSignature signature = TransactionSignature.decodeFromNeoscoin(signatureBytes, true);
        for (int i = 0 ; i < numKeys ; i++) {
     ...
     * logic is defined by <a href="https://github.com/neoscoin/bips/blob/master/bip-0016.mediawiki">BIP 16</a>.</p>
     *
     * <p>neoscoinj does not support creation of P2SH transactions today. The goal of P2SH is to allow short addresses
     ...
     * {@link org.neoscoinj.core.TransactionInput#verify(org.neoscoinj.core.TransactionOutput)} or
     * {@link org.neoscoinj.script.Script#correctlySpends(org.neoscoinj.core.Transaction, long, Script)}. This method
    ...
    private static void executeCheckSig(Transaction txContainingThis, int index, Script script, LinkedList<byte[]> stack,
        ...
        try {
            TransactionSignature sig  = TransactionSignature.decodeFromNeoscoin(sigBytes, false);
    ...
    private static int executeMultiSig(Transaction txContainingThis, int index, Script script, LinkedList<byte[]> stack,
        ...
        while (sigs.size() > 0) {
            ...
            try {
                TransactionSignature sig = TransactionSignature.decodeFromNeoscoin(sigs.getFirst(), false);
                Sha256Hash hash = txContainingThis.hashForSignature(index, connectedScript, (byte) sig.sighashFlags);
    ...
    public void correctlySpends(Transaction txContainingThis, long scriptSigIndex, Script scriptPubKey,
        ...
        try {
            txContainingThis = new Transaction(txContainingThis.getParams(), txContainingThis.neoscoinSerialize());
    ...
    /**
     * Get the {@link org.neoscoinj.script.Script.ScriptType}.
