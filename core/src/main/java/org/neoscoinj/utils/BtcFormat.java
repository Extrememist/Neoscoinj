...
import org.neoscoinj.utils.NeosAutoFormat.Style;
import static org.neoscoinj.utils.NeosAutoFormat.Style.*;
 ...
 * representations of Neoscoin monetary values.
 *
 ...
 * represents a Neoscoin monetary value.  Some of the features offered for doing this
 * are: <ol>
 ...
 *       {@link NeosFormat#format(Object)} method. 
 *   <li>Parse a value by passing a <code>String</code>-type
 *       representation of it to the {@link NeosFormat#parse(String)} method.</ol>
 *
 * <p>For example, depending on your locale, values might be formatted
 * and parsed as follows:
 *
 * <blockquote><pre>
 * NeosFormat f = NeosFormat.getInstance();
 * String c = f.format(Coin.COIN);                <strong>// "NEOS 1.00"</strong>
 * String k = f.format(Coin.COIN.multiply(1000)); <strong>// "NEOS 1,000.00"</strong>
 * String m = f.format(Coin.COIN.divide(1000));   <strong>// "mNEOS 1.00"</strong>
 ...
 * be formatted, {@link NeosAutoFormat}, and another that formats any value in units of a
 * fixed, specified denomination, {@link NeosFixedFormat}.  
 *
 ...
 * values of {@link NeosAutoFormat.Style}.  There are two styles constants: {@link
 * NeosAutoFormat.Style#CODE} (the default), and {@link NeosAutoFormat.Style#SYMBOL}.  The
 * difference is that the <code>CODE</code> style uses an internationally-distinct currency
 * code, such as <code>"NEOS"</code>, to indicate the units of denomination, while the
 * <code>SYMBOL</code> style uses a possibly-ambiguous currency symbol such as
 * <code>"฿"</code>.
 *
 * <p>The denomination used when formatting will be either neoscoin, millicoin
 ...
 * locale, a value of one neoscoin might be formatted as <pre>฿1.00</pre> where a value
 ...
 * increasing precision to convert the representation of a given quantity of neoscoins into a
 * representation of the same value denominated in the formatter's units.  For example, a scale
 * value of <code>3</code> specifies a denomination of millineoscoins, because to represent
 * <code>1.0000 NEOS</code>, or one neoscoin, in millineoscoins, one shifts the decimal point
 * three places, that is, to <code>1000.0 mNEOS</code>.
 *
 * <h3>Construction</h3>
 *
 * There are two ways to obtain an instance of this class: <ol>
 *   <li>Use one of the factory methods; or
 *   <li>Use a {@link NeosFormat.Builder} object.</ol>
 *
 ...
 * methods of this abstract base class <code>NeosFormat</code>.  There
 ...
 * automatic-denominating <code>NeosAutoFormat</code> instance for your
 ...
 * <blockquote><pre>
 * NeosFormat f = NeosFormat.getInstance();
 * String s = f.format(Coin.COIN); <strong>// "NEOS 1.00"</strong>
 * </pre></blockquote>
 *
 ...
 * the {@link NeosFixedFormat} instance that is returned, and thus will
 * determine its denomination.  For example, if you want to format
 * values in units of microneoscoins:
 *
 * <blockquote><pre>NeosFormat m = NeosFormat.getInstance(6);
 ...
 *
 * <blockquote><pre>NeosFormat milliFormat = NeosFormat.getInstance(MILLICOIN_SCALE);</pre></blockquote>
 *
 * <p>Alternatively, if the type of the first argument to
 * <code>getInstance()</code> is one of the enumerated values of the
 * {@link NeosAutoFormat.Style} type, either <code>CODE</code> or
 * <code>SYMBOL</code>, then you will get a {@link NeosAutoFormat}
 * instance that uses either a currency code or symbol, respectively,
 * to indicate the results of its auto-denomination.
 *
 * <blockquote><pre>
 * NeosFormat s = NeosFormat.getInstance(SYMBOL);
 ...
 * differently the same one-neoscoin value:
 *
 * <blockquote><pre>
 * <strong>// Next line returns "1,00 NEOS"</strong>
 * NeosFormat.getInstance(Locale.GERMANY).format(Coin.COIN);
 * <strong>// Next line returns "1,00 ฿"</strong>
 * NeosFormat.getInstance(SYMBOL, Locale.GERMANY).format(Coin.COIN);
 * <strong>// Next line returns "1.000,00"</strong>
 * NeosFormat.getMilliInstance(Locale.GERMANY).format(Coin.COIN);
 * <strong>// Next line returns "10.000,00"</strong>
 * NeosFormat.getInstance(4, Locale.GERMANY).format(Coin.COIN);
 * </pre></blockquote>
 *
 ...
 *
 * A new {@link NeosFormat.Builder} instance is returned by the {@link #builder()} method.  Such
 * an object has methods that set the configuration parameters of a <code>NeosFormat</code>
 * object.  Its {@link Builder#build()} method constructs and returns a <code>NeosFormat</code> instance
 ...
 *
 * <blockquote><pre>
 * NeosFormat f = NeosFormat.builder().style(CODE).code("XBT").build();
 * String out = f.format(COIN); <strong>// "XBT 1.00"</strong>
 * </pre></blockquote>
 *
 * <p>See the documentation of the {@link NeosFormat.Builder} class for details.
 *
 * <h3>Formatting</h3>
 *
 * <p>You format a Neoscoin monetary value by passing it to the {@link NeosFormat#format(Object)}
 * method.  This argument can be either a {@link org.neoscoinj.core.Coin}-type object or a
 ...
 * number of neoscoins.  A value having a fractional amount of satoshis is rounded to the
 ...
 * {@link NeosFixedFormat} also accept a variable-length sequence of additional <code>int</code>
 ...
 *
 * <blockquote><pre>
 * NeosFormat f = NeosFormat.getCoinInstance();
 ...
 * {@link NeosFixedFormat#REPEATING_PLACES}, {@link
 * NeosFixedFormat#REPEATING_DOUBLETS} and {@link
 * NeosFixedFormat#REPEATING_TRIPLETS}.  These signify repeating groups
 ...
 *
 * <blockquote><pre>
 * NeosFormat de = NeosFormat.getInstance(Locale.GERMANY);
 * FieldPosition currField = new FieldPosition(NumberFormat.Field.CURRENCY);
 * <strong>// next line formats the value as "987.654.321,23 µNEOS"</strong>
 * String output = de.format(valueOf(98765432123L), new StringBuffer(), currField);
 * <strong>// next line sets variable currencyCode to "µNEOS"</strong>
 ...
 *
 * <blockquote><pre>
 * NeosFixedFormat kilo = (NeosFixedFormat)NeosFormat(-3); <strong>// scale -3 for kilocoins</strong>
 * Coin value = Coin.parseCoin("1230");
 * <strong>// variable coded will be set to "kNEOS 1.23"</strong>
 ...
 * NeosFormat(4).code(); <strong>// unnamed denomination has no code; raises exception</strong>
 * </pre></blockquote>
 *
 ...
 *                SATOSHI};
 * NeosFormat f = NeosFormat.getCoinInstance(2, REPEATING_PLACES);
 ...
 * NeosFixedFormat} and {@link NeosAutoFormat} is analogous to the difference in formatting
 * behavior between instances of those classes.  Instances of {@link NeosAutoFormat} recognize
 ...
 * instances of {@link NeosFixedFormat} by default recognize no codes nor symbols, but rather
 ...
 * NeosFixedFormat} can be overridden by setting a parsing pattern that includes a currency sign
 * using the {@link NeosFormat.Builder#pattern()} method.
 *
 * <p>The {@link NeosAutoFormat#parse(String)}</code> method of {@link NeosAutoFormat} (and of
 * {@link NeosAutoFormat} configured with applicable non-default pattern) will recognize a
 ...
 * <code>µNEOS</code> or other appropriate permutations of those characters.  Additionally, if
 * either or both of a custom currency code or symbol is configured using {@link
 * NeosFormat.Builder#code()} or {@link NeosFormat.Builder.code()}, then such code or symbol will
 ...
 * <code>NeosFormat</code> instance, on the other hand, whether or not the units indicator must
 * be separated by a space from the number is determined by the locale.  The {@link
 * NeosFormat#pattern()} method returns a representation of the pattern that
 * can be examined to determine whether a space must separate currency signs from numbers in
 * parsed <code>String</code>s.
 *
 * <p>When parsing, if the currency-units indicator is absent, then a {@link NeosAutoFormat}
 * instance will infer a denomination of neoscoins while a {@link NeosFixedFormat} will infer the
 ...
 * @see          org.neoscoinj.core.Coin
 */

public abstract class NeosFormat extends Format {
    ...
    /** The conventional international currency code for neoscoins: "NEOS" */
    private static final String COIN_CODE = "NEOS";
    /** The default currency symbols for neoscoins */
    ...
    /* Scale is the number of decimal-places difference from same value in neoscoins */
    /** A constant useful for specifying a denomination of neoscoins, the <code>int</code> value
     *  <code>0</code>. */
    public static final int COIN_SCALE = 0;

    /** A constant useful for specifying a denomination of millineoscoins, the <code>int</code>
     *  value <code>3</code>. */
    public static final int MILLICOIN_SCALE = 3;

    /** A constant useful for specifying a denomination of microneoscoins, the <code>int</code>
     *  value <code>6</code>. */
    ...
    /**
     * <p>This class constructs new instances of {@link NeosFormat}, allowing for the
     * configuration of those instances before they are constructed.  After obtaining a
     * <code>Builder</code> object from the {@link NeosFormat#builder()} method, invoke the
     * necessary setter methods to obtain your desired configuration.  Finaly, the {@link
     * #build()} method returns a new <code>NeosFormat</code> object that has the specified
     * configuration.
     *
     * <p>All the setter methods override defaults.  Invoking <code>build()</code> without invoking any
     * of the setting methods is equivalent to invoking {@link NeosFormat#getInstance()} with no arguments.
     *
    ...
    public static class Builder {
        ...
        private enum Variant {
            AUTO { @Override NeosFormat newInstance(Builder b) {
                       return getInstance(b.style, b.locale, b.minimumFractionDigits);
                   }},
            FIXED,
            UNSET;
            NeosFormat newInstance(Builder b) {
        ...
        private Style style = NeosAutoFormat.Style.CODE;
        private int scale = 0;
        private String symbol = "",code = "",pattern = "",localizedPattern = "";

        private Builder() {}

        /** Specify the new <code>NeosFormat</code> is to be automatically-denominating.
         * The argument determines which of either codes or symbols the new <code>NeosFormat</code>
        ...
        public Builder style(NeosAutoFormat.Style val) {
         ...
         * appropriately for the context of generating {@link NeosAutoFormat} instances.
         *
         ...
         *  denominated in neoscoins.  For example, a denomination of millineoscoins is specified
         *  with a value of <code>3</code>.
         *
         * <p>The <code>NeosFormat</code> class provides appropriately named
         * <code>int</code>-type constants for the three common values, {@link NeosFormat#COIN_SCALE},
         * {@link NeosFormat#MILLICOIN_SCALE} {@link NeosFormat#MICROCOIN_SCALE}.
         *
         * <p>If neither this method nor {@link #style(NeosAutoFormat.Style)} is invoked on a
         * <code>Builder</code>, then the <code>NeosFormat</code> will default to a
         * fixed-denomination of neoscoins, equivalent to invoking this method with an argument
         * of <code>0</code>. */
         ...
         *  class.
         *
         *  <p>The pattern is localized according to the locale of the <code>NeosFormat</code>
         *  instance, the symbols for which can be examined by inspecting the {@link
         *  java.text.DecimalFormatSymbols} object returned by {@link NeosFormat#symbols()}.
        ...
        /** Return a new {@link NeosFormat} instance.  The object returned will be configured according
         *  to the state of this <code>Builder</code> instance at the time this method is invoked. */
        public NeosFormat build() {
            NeosFormat f = variant.newInstance(this);
    ...
    protected NeosFormat(DecimalFormat numberFormat, int minDecimals, List<Integer> groups) {
     ...
     * currency code, for example "<code>NEOS</code>", to indicate that denomination.  The
     ...
     */
    public static NeosFormat getInstance() { return getInstance(defaultLocale()); }
     ...
     */
    public static NeosFormat getSymbolInstance() { return getSymbolInstance(defaultLocale()); }

    /**
     * Return a new auto-denominating instance that will indicate units using a currency
     * code, for example, <code>"NEOS"</code>.  Formatting and parsing will be done
     * according to the default locale.
     */
    public static NeosFormat getCodeInstance() { return getCodeInstance(defaultLocale()); }
     ...
     * locale.
     */
    public static NeosFormat getSymbolInstance(int fractionPlaces) {
     ...
     * <code>"NEOS"</code>.  The returned object will format the fraction-part of numbers using
     * the given number of decimal places, or fewer as necessary to avoid giving a place to
     * fractional satoshis.  Formatting and parsing will be done according to the default
     * locale.
     */
    public static NeosFormat getCodeInstance(int minDecimals) {
     ...
     * units using a currency code, for example, <code>"mNEOS"</code>.
     */
    public static NeosFormat getInstance(Locale locale) { return getCodeInstance(locale); }
     ...
     * units using a currency code, for example, <code>"mNEOS"</code>.
     */
    public static NeosFormat getCodeInstance(Locale locale) { return getInstance(CODE, locale); }
     ...
     * <code>"mNEOS"</code>.  The returned object will format the fraction-part of numbers using
     * the given number of decimal places, or fewer as necessary to avoid giving a place to
     * fractional satoshis.
     */
    public static NeosFormat getInstance(Locale locale, int minDecimals) {
     ...
     * fractional satoshis.
     */
    public static NeosFormat getCodeInstance(Locale locale, int minDecimals) {
     ...
     */
    public static NeosFormat getSymbolInstance(Locale locale) {
     ...
     * fractional satoshis.
     */
    public static NeosFormat getSymbolInstance(Locale locale, int fractionPlaces) {
     ...
     * <code>"฿"</code>, or code, such as <code>"mNEOS"</code>, depending on the value of
     * the argument.  Formatting and parsing will be done according to the default locale.
     */
    public static NeosFormat getInstance(Style style) { return getInstance(style, defaultLocale()); }
     ...
     * <code>"mNEOS"</code>, depending on the value of the first argument.  The returned object
     ...
     */
    public static NeosFormat getInstance(Style style, int fractionPlaces) {
     ...
     * "<code>NEOS</code>", or symbol, such as "<code>฿</code>", depending on the value
     ...
     */
    public static NeosFormat getInstance(Style style, Locale locale) {
     ...
     * such as <code>"mNEOS"</code>, or symbol, such as "<code>฿</code>",
     ...
     * fractional satoshis.
     */
    public static NeosFormat getInstance(Style style, Locale locale, int fractionPlaces) {
	return new NeosAutoFormat(locale, style, fractionPlaces);
    }
     ...
     */
    public static NeosFormat getCoinInstance() { return getCoinInstance(defaultLocale()); }
     ...
     */
    public static NeosFormat getCoinInstance(int minFractionPlaces, int... groups) {
     ...
     */
    public static NeosFormat getCoinInstance(Locale locale) {
        return getInstance(COIN_SCALE, locale, 2);
    }

    /**
     * Return a newly-constructed instance for the given locale that will format
     * values in terms of neoscoins, with the given minimum number of fractional
     ...
     */
    public static NeosFormat getCoinInstance(Locale locale, int scale, int... groups) {
     ...
     */
    public static NeosFormat getMilliInstance() { return getMilliInstance(defaultLocale()); }
     ...
     * necessary.
     */
    public static NeosFormat getMilliInstance(Locale locale) {
     ...
     */
    public static NeosFormat getMilliInstance(int scale, int... groups) {
     ...
     */
    public static NeosFormat getMilliInstance(Locale locale, int scale, int... groups) {
     ...
     * necessary.
     */
    public static NeosFormat getMicroInstance() { return getMicroInstance(defaultLocale()); }
     ...
     * necessary.
     */
    public static NeosFormat getMicroInstance(Locale locale) {
     ...
     * necessary to avoid rounding, to a limiting precision of satoshis.
     */
    public static NeosFormat getMicroInstance(int scale, int... groups) {
     ...
     * limiting precision of satoshis.
     */
    public static NeosFormat getMicroInstance(Locale locale, int scale, int... groups) {
     ...
     * limiting precision of satoshis.
     */
    public static NeosFormat getInstance(int scale, int minDecimals, int... groups) {
     ...
     */
    public static NeosFormat getInstance(int scale) {
     ...
     */
    public static NeosFormat getInstance(int scale, Locale locale) {
     ...
     */
    public static NeosFormat getInstance(int scale, Locale locale, int minDecimals, int... groups) {
     ...
     */
    public static NeosFormat getInstance(int scale, Locale locale, int minDecimals, List<Integer> groups) {
        return new NeosFixedFormat(locale, scale, minDecimals, groups);
    }
    ...
    /**
     * Formats a neoscoin monetary value and returns an {@link java.text.AttributedCharacterIterator}.
    ...
    /**
     * Formats a neoscoin value as a number and possibly a units indicator and appends the
     ...
     * number of neoscoins, rounded to the nearest satoshi as necessary.
     *
    ...
    /**
     * Formats a neoscoin value as a number and possibly a units indicator to a
     ...
     * <code>BigDecimal</code> is interpreted as that number of neoscoins, rounded to the
     ...
     * is the size of the decimal-place shift between the given Neoscoin-value denominated in
     * neoscoins and that same value as formatted.  A fixed-denomination formatter will ignore
     * the arguments.
     *
    ...
    /**
     * Takes a neoscoin monetary value that the client wants to format and returns the number of
    ...
    /**
     * Takes an object representing a neoscoin quantity of any type the
	...
	BigInteger satoshis;
        // the value might be neoscoins or satoshis
    ...
    /**
      * Parse a <code>String</code> representation of a Neoscoin monetary value.  Returns a
      * {@link org.neoscoinj.core.Coin} object that represents the parsed value.
      * @see java.text.NumberFormat */
    @Override
    ...
    /** Parse a <code>String</code> representation of a Neoscoin monetary value.  If this
     * object's pattern includes a currency sign, either symbol or code, as by default is true
     * for instances of {@link NeosAutoFormat} and false for instances of {@link
     * NeosFixedFormat}, then denominated (i.e., prefixed) currency signs in the parsed String
     ...
     * String, then the number is interpreted as a quatity of neoscoins.
     * <p>If the pattern contains neither a currency symbol nor sign, then instances of {@link
     * NeosAutoFormat} will interpret the parsed number as a quantity of neoscoins, and instances
     * of {@link NeosAutoFormat} will interpret the number as a quantity of that instance's
     * configured denomination, which can be ascertained by invoking the {@link
     * NeosFixedFormat#symbol()} or {@link NeosFixedFormat#code()} method.
     *
    ...
    /** Return the unprefixed currency symbol for neoscoins configured for this object.  The
    ...
    /** Return the unprefixed international currency code for neoscoins configured for this
    ...
    @Override public boolean equals(Object o) {
        ...
        if (!(o instanceof NeosFormat)) return false;
        NeosFormat other = (NeosFormat)o;
