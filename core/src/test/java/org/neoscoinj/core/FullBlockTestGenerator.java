...
import org.neoscoinj.core.Transaction.SigHash;
...
import static org.neoscoinj.script.ScriptOpCodes.*;
 ...
 *    rethink your life. Seriously, why are you reimplementing Neoscoin consensus rules? Instead, go work on making
 *    Neoscoin Core consensus rules a shared library and use that. Seriously, you wont get it right, and starting with
 *    this tester as a way to try to do so will simply end in pain and lost coins. SERIOUSLY, JUST STOP!
 *
 * b) Neoscoin Core is faling some test in here and you're wondering what test is causing failure. Just stop. There is no
...
public class FullBlockTestGenerator {
    // Used by NeoscoindComparisonTool and AbstractFullPrunedBlockChainTest to create test cases
    ...
    public RuleList getBlocksToTest(boolean runBarelyExpensiveTests, boolean runExpensiveTests, File blockStorageFile) throws ScriptException, ProtocolException, IOException {
            ...
            public boolean add(Rule element) {
                if (outStream != null && element instanceof BlockAndValidity) {
                    try {
                        ...
                        byte[] block = ((BlockAndValidity)element).block.neoscoinSerialize();
        ...
        // Extend the b24 chain to make sure neoscoind isn't accepting b24
        ...
        // Extend the b26 chain to make sure neoscoind isn't accepting b26
        ...
        // Extend the b28 chain to make sure neoscoind isn't accepting b28
        ...
        Block b56;
        try {
            b56 = new Block(params, b57.block.neoscoinSerialize());
        ...
        Block b56p2;
        try {
            b56p2 = new Block(params, b57p2.block.neoscoinSerialize());
        } catch (ProtocolException e) {
            throw new RuntimeException(e); // Cannot happen.
        }
        ...
        Block b64; NewBlock b64Original;
        {
            ...
            for (Transaction transaction : b64Original.block.getTransactions())
                transaction.neoscoinSerialize(stream);
            ...
            checkState(Arrays.equals(stream.toByteArray(), b64.neoscoinSerialize()));
        ...
        Block b71 = new Block(params, b72.block.neoscoinSerialize());
    ...
    class BlockAndValidity extends Rule {
        ...
        boolean sendOnce; // We can throw away the memory for this block once we send it the first time (if neoscoind asks again, its broken)
