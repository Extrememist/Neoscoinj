...
public class NeoscoinSerializerTest {
    ...
    public void testAddr() throws Exception {
        NeoscoinSerializer bs = new NeoscoinSerializer(MainNetParams.get());
        // the actual data from https://en.neoscoin.it/wiki/Protocol_specification#addr
    ...
    public void testLazyParsing()  throws Exception {
        NeoscoinSerializer bs = new NeoscoinSerializer(MainNetParams.get(), true, false);
    ...
    private void testCachedParsing(boolean lazy)  throws Exception {
        NeoscoinSerializer bs = new NeoscoinSerializer(MainNetParams.get(), lazy, true);
    ...
    @Test
    public void testNeoscoinPacketHeader() {
        try {
            new NeoscoinSerializer.NeoscoinPacketHeader(ByteBuffer.wrap(new byte[]{0}));
            fail();
        ...
        try {
            new NeoscoinSerializer.NeoscoinPacketHeader(ByteBuffer.wrap(wrongMessageLength));
            fail();
    ...
    public void testSeekPastMagicBytes() {
        ...
        try {
            new NeoscoinSerializer(MainNetParams.get()).seekPastMagicBytes(ByteBuffer.wrap(brokenMessage));
            fail();
        } catch (BufferUnderflowException e) {
            // expected
        }
