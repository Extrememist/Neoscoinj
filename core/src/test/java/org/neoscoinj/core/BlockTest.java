...
import org.neoscoinj.params.TestNet2Params;
...
public class BlockTest {
    ...
    public void testHeaderParse() throws Exception {
        ...
        Block reparsed = new Block(params, header.neoscoinSerialize());
    ...
    public void testBitCoinSerialization() throws Exception {
        ...
        // of transactions.
        Block block = new Block(params, blockBytes);
        assertTrue(Arrays.equals(blockBytes, block.neoscoinSerialize()));
    }
    ...
    public void testJavaSerialiazation() throws Exception {
        ...
        // Note that this will actually check the transactions are equal by doing neoscoin serialization and checking
    ...
    public void testUpdateLength() {
        ...
        assertEquals(block.neoscoinSerialize().length, block.length);
        final int origBlockLen = block.length;
        Transaction tx = new Transaction(params);
        // this is broken until the transaction has > 1 input + output (which is required anyway...)
        //assertTrue(tx.length == tx.neoscoinSerialize().length && tx.length == 8);
        ...
        assertEquals(tx.neoscoinSerialize().length, tx.length);
        ...
        assertEquals(block.length, block.neoscoinSerialize().length);
        assertEquals(block.length, origBlockLen + tx.length);
        assertEquals(tx.length, origTxLength - 1);
