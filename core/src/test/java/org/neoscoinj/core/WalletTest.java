...
import org.neoscoinj.core.Wallet.SendRequest;
import org.neoscoinj.crypto.*;
import org.neoscoinj.script.Script;
import org.neoscoinj.script.ScriptBuilder;
import org.neoscoinj.script.ScriptOpCodes;
import org.neoscoinj.signers.StatelessTransactionSigner;
...
import org.neoscoinj.testing.*;
import org.neoscoinj.utils.ExchangeRate;
import org.neoscoinj.utils.Fiat;
...
import static org.neoscoinj.core.Utils.HEX;
import static org.neoscoinj.testing.FakeTxBuilder.*;
...
public class WalletTest extends TestWithWallet {
    ...
    private void basicSpendingCommon(Wallet wallet, Address toAddress, Address destination, Wallet encryptedWallet) throws Exception {
        ...
        // will attach a small fee. Because the Neoscoin protocol makes it difficult to determine the fee of an
    ...
    public void balances() throws Exception {
        ...
        Transaction send1 = wallet.createSend(new ECKey().toAddress(params), valueOf(0, 10));
        // Reserialize.
        Transaction send2 = new Transaction(params, send1.neoscoinSerialize());
    ...
    public void isConsistent_duplicates() throws Exception {
        ...
        Transaction txClone = new Transaction(params, tx.neoscoinSerialize());
        try {
    ...
    public void doubleSpendUnspendsOtherInputs() throws Exception {
        ...
        send2 = new Transaction(params, send2.neoscoinSerialize());
    ...
    public void doubleSpends() throws Exception {
        ...
        byte[] buf = send1.neoscoinSerialize();
        buf[43] = 0;  // Break the signature: neoscoinj won't check in SPV mode and this is easier than other mutations.
    ...
    public void pending1() throws Exception {
        ...
        final Transaction t1Copy = new Transaction(params, t1.neoscoinSerialize());
    ...
    public void spendToSameWallet() throws Exception {
        ...
        assertEquals(ZERO, wallet.getBalance());
        sendMoneyToWallet(outbound1, AbstractBlockChain.NewBlockType.BEST_CHAIN);
        // We should have a balance of 1 NEOS after the block is received.
        assertEquals(coin1, wallet.getBalance());
    }
    ...
    public void watchingScriptsBloomFilter() throws Exception {
        ...
        assertFalse(wallet.getBloomFilter(1e-12).contains(outPoint.neoscoinSerialize()));

        wallet.receiveFromBlock(t1, b1, BlockChain.NewBlockType.BEST_CHAIN, 0);
        assertTrue(wallet.getBloomFilter(1e-12).contains(outPoint.neoscoinSerialize()));
    }
    ...
    public void marriedKeychainBloomFilter() throws Exception {
        ...
        assertFalse(wallet.getBloomFilter(0.001).contains(outPoint.neoscoinSerialize()));

        wallet.receiveFromBlock(t1, b1, BlockChain.NewBlockType.BEST_CHAIN, 0);
        assertTrue(wallet.getBloomFilter(0.001).contains(outPoint.neoscoinSerialize()));
    }

    @Test
    public void autosaveImmediate() throws Exception {
        // Test that the wallet will save itself automatically when it changes.
        File f = File.createTempFile("neoscoinj-unit-test", null);
    ...
    public void opReturnTwoOutputsTest() throws Exception {
        // Tests sending transaction where one output transfers NEOS, the other one writes OP_RETURN.
        receiveATransaction(wallet, myAddress);
        Address notMyAddr = new ECKey().toAddress(params);
        Transaction tx = new Transaction(params);
        Coin messagePrice = Coin.ZERO;
    ...
    public void feeSolverAndCoinSelectionTest() throws Exception {
        ...
        assertTrue(request15.tx.neoscoinSerialize().length > 1000);
        ...
        assertTrue(request16.tx.neoscoinSerialize().length > 1000);
        ...
        int theoreticalMaxLength17 = request17.tx.neoscoinSerialize().length + myKey.getPubKey().length + 75;
        ...
        Transaction spend17 = request17.tx;
        {
            ...
            final int length = spend17.neoscoinSerialize().length;
        ...
        int theoreticalMaxLength18 = spend18.neoscoinSerialize().length + myKey.getPubKey().length + 75;
        ...
        assertTrue(spend18.neoscoinSerialize().length >= 998);
        assertTrue(spend18.neoscoinSerialize().length <= 1001);
        ...
        assertTrue(request26.tx.neoscoinSerialize().length > 1000);
    ...
    public void basicCategoryStepTest() throws Exception {
        ...
        request4.feePerKb = Transaction.REFERENCE_DEFAULT_MIN_TX_FEE.divide(request3.tx.neoscoinSerialize().length);
        wallet.completeTx(request4);
    ...
    public void completeTxPartiallySignedWithDummySigs() throws Exception {
        byte[] dummySig = TransactionSignature.dummy().encodeToNeoscoin();
