...
import static org.neoscoinj.testing.FakeTxBuilder.createFakeBlock;
...
public class LazyParseByteCacheTest {
    ...
    public void setUp() throws Exception {
        ...
        NeoscoinSerializer bs = new NeoscoinSerializer(unitTestParams);
        
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
        bs.serialize(tx1, bos);
        tx1BytesWithHeader = bos.toByteArray();
        tx1Bytes = tx1.neoscoinSerialize();
        
        bos.reset();
        bs.serialize(tx2, bos);
        tx2BytesWithHeader = bos.toByteArray();
        tx2Bytes = tx2.neoscoinSerialize();
        
        bos.reset();
        bs.serialize(b1, bos);
        b1BytesWithHeader = bos.toByteArray();
        b1Bytes = b1.neoscoinSerialize();
    }
    ...
    public void testBlock(byte[] blockBytes, boolean isChild, boolean lazy, boolean retain) throws Exception {
        ...
        //message structure.
        NeoscoinSerializer bsRef = new NeoscoinSerializer(unitTestParams, false, false);
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
        
        NeoscoinSerializer bs = new NeoscoinSerializer(unitTestParams, lazy, retain);
        Block b1;
        Block bRef;
        b1 = (Block) bs.deserialize(ByteBuffer.wrap(blockBytes));
        bRef = (Block) bsRef.deserialize(ByteBuffer.wrap(blockBytes));
        
        //verify our reference NeoscoinSerializer produces matching byte array.
        bos.reset();
    ...
    public void testTransaction(NetworkParameters params, byte[] txBytes, boolean isChild, boolean lazy, boolean retain) throws Exception {
        ...
        //reference serializer to produce comparison serialization output after changes to
        //message structure.
        NeoscoinSerializer bsRef = new NeoscoinSerializer(params, false, false);
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
        
        NeoscoinSerializer bs = new NeoscoinSerializer(params, lazy, retain);
        Transaction t1;
        Transaction tRef;
    ...
    private void serDeser(NeoscoinSerializer bs, Message message, byte[] sourceBytes, byte[] containedBytes, byte[] containingBytes) throws Exception {
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
