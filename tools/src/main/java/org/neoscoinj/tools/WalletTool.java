...
package org.neoscoinj.tools;

import org.neoscoinj.core.*;
import org.neoscoinj.core.Wallet.BalanceType;
import org.neoscoinj.crypto.DeterministicKey;
import org.neoscoinj.crypto.KeyCrypterException;
import org.neoscoinj.crypto.MnemonicCode;
import org.neoscoinj.crypto.MnemonicException;
import org.neoscoinj.net.discovery.DnsDiscovery;
...
import org.neoscoinj.protocols.payments.PaymentProtocol;
import org.neoscoinj.protocols.payments.PaymentProtocolException;
import org.neoscoinj.protocols.payments.PaymentSession;
import org.neoscoinj.store.*;
import org.neoscoinj.uri.NeoscoinURI;
import org.neoscoinj.uri.NeoscoinURIParseException;
...
import org.neoscoinj.wallet.MarriedKeyChain;
...
import static org.neoscoinj.core.Coin.parseCoin;
import static com.google.common.base.Preconditions.checkNotNull;

/**
 * A command line tool for manipulating wallets and working with Neoscoin.
 */
public class WalletTool {
    ...
    private static org.neoscoin.protocols.payments.Protos.PaymentRequest paymentRequest;
    ...
    public static class Condition {
        ...
        public boolean matchNeoscoins(Coin comparison) {
            try {
    ...
    private static void sendPaymentRequest(String location, boolean verifyPki) {
        if (location.startsWith("http") || location.startsWith("neoscoin")) {
            try {
                ListenableFuture<PaymentSession> future;
                if (location.startsWith("http")) {
                    future = PaymentSession.createFromUrl(location, verifyPki);
                } else {
                    NeoscoinURI paymentRequestURI = new NeoscoinURI(location);
                    future = PaymentSession.createFromNeoscoinUri(paymentRequestURI, verifyPki);
                }
                ...
                System.err.println("Invalid neoscoin uri: " + e.getMessage());
                System.exit(1);
            } catch (InterruptedException e) {
                // Ignore.
            } catch (ExecutionException e) {
            ...
            try {
                paymentRequest = org.neoscoin.protocols.payments.Protos.PaymentRequest.newBuilder().mergeFrom(stream).build();
            } catch(IOException e) {
    ...
    private static void wait(WaitForEnum waitFor) throws BlockStoreException {
        ...
        switch (waitFor) {
            ...
            case BALANCE:
                ...
                if (condition.matchNeoscoins(wallet.getBalance(Wallet.BalanceType.ESTIMATED))) {
                    latch.countDown();
                    break;
                }
                wallet.addEventListener(new AbstractWalletEventListener() {
                    ...
                    public synchronized void onChange() {
                        ...
                        if (condition.matchNeoscoins(balance)) {
    ...
    private static void deleteKey() {
        ...
        if (pubkey != null) {
            ...
            try {
                ...
                System.err.println(addr + " does not parse as a Neoscoin address of the right network parameters.");
                return;
            }
        }
        if (key == null) {
