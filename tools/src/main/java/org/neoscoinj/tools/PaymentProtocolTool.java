/*
 * Copyright 2014 The neoscoinj team
 *
...
/** Takes a URL or neoscoin URI and prints information about the payment request. */
public class PaymentProtocolTool {
    public static void main(String[] args) {
        if (args.length < 1) {
            System.err.println("Provide a neoscoin URI or URL as the argument.");
            return;
        }
        dump(args[0]);
    }
    ...
    private static void dump(String arg) {
        try {
            ...
            } else if (uri.getScheme().equals("neoscoin")) {
                NeoscoinURI bcuri = new NeoscoinURI(arg);
                final String paymentRequestUrl = bcuri.getPaymentRequestUrl();
                if (paymentRequestUrl == null) {
                    System.err.println("No r= param in neoscoin URI");
                    return;
                }
                session = PaymentSession.createFromNeoscoinUri(bcuri).get();
            } else {
            ...
            StringBuilder output = new StringBuilder(
                    format("Neoscoin payment request, version %d%nDate: %s%n", version, session.getDate()));
